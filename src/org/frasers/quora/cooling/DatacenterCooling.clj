...
(defn seek-end [datacentermap mypath x y]
  (let [paths (for [dx [(dec x) x (inc x)] dy [(dec y) y (inc y)]
                   :when (and
                            (not (and (= dx x) (= dy y)))
                            (not (neg? dx))
                            (not (neg? dy))
                            (= 0 (datacentermap [dx dy])))]
                [dx dy])]
    ...
    ; Step 3 - would we just recur for one of the branches to avoid creating too many agents?
    )
  )
...
(defn -main [w h datacenterinput]
  (let [datacentermap (zipmap
    ...
    (prn datacentermap)
    (seek-end datacentermap [[startx starty]] startx starty)
  ))
